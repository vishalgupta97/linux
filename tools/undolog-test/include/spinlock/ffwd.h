...
#define ENABLE_JUMP 0       // Context switch from client to client
#define ENABLE_PREFETCH 1   // Prefetch before context switch
#define ENABLE_RESPONSE_BATCHING 0

#define DEBUG_DELEGATION 0 //1
...
struct delegation_server {
	union {
		struct {
			...
			uint32_t cur_client_cpu_id;
			uint32_t prev_client_cpu_id;
			uint32_t next_client_cpu_id;
			uint32_t cur_socket_id;
			uint32_t waiters_combined;
		};
		char alignment[CACHELINE_ALIGNEMENT];
	};
