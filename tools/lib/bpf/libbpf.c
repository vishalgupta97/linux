...
int bpf_object__attach_skeleton(struct bpf_object_skeleton *s)
{
	...
	for (i = 0; i < s->map_cnt; i++) {
		...
		/* cache_ext struct_ops maps are attached to a specific cgroup
		 * explicitly via bpf_map__attach_cache_ext_ops(); the generic
		 * struct_ops auto-attach below has no cgroup and would fail, so
		 * skip them here.
		 */
		if (map->st_ops) {
			const struct btf_type *st_t =
				btf__type_by_id(map->obj->btf, map->st_ops->type_id);
			const char *st_tname = st_t ?
				btf__name_by_offset(map->obj->btf, st_t->name_off) : NULL;

			if (st_tname && strcmp(st_tname, "cache_ext_ops") == 0)
				continue;
		}
