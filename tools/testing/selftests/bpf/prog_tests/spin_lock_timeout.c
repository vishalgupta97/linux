...
static void *deadlock_thread1(void *arg)
{
	...
	ASSERT_OK(0, "starting test_deadlock_thread1");
	err = bpf_prog_test_run_opts(prog_fd, &opts);
	ASSERT_OK(err, "finished test_deadlock_thread1");
	return NULL;
}
...
static void *deadlock_thread2(void *arg)
{
	...
	ASSERT_OK(0, "starting test_deadlock_thread2");
	err = bpf_prog_test_run_opts(prog_fd, &opts);
	ASSERT_OK(err, "finished test_deadlock_thread2");
	return NULL;
}
...
static void test_deadlock_abba(void)
{
	...
	if (__write_sysctl(5000) < 0) {  /* 500ms timeout */
		test__skip();
		return;
	}
