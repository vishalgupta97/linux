...
extern void __raw_spin_lock_init_static_fds(raw_spinlock_t *lock,
					     const char *name,
					     struct fds_lock_key *key,
...
static inline void do_raw_spin_lock_fds(raw_spinlock_t *lock, struct fds_lock_key *key) __acquires(lock)
{
	__acquire(lock);
	if (key)
		arch_spin_lock_fds(&lock->raw_lock, key);
	else
		arch_spin_lock(&lock->raw_lock);
	mmiowb_spin_lock();
}
...
#define spin_lock_init_disable_fds(_lock)                               \
	do {                                                            \
		...
		__raw_spin_lock_init_static_fds(spinlock_check(_lock), \
						 #_lock, &__key, FDS_DISABLE);       \
	} while (0)

#define spin_lock_init_tclock_fds(_lock)                               \
	do {                                                            \
		static struct fds_lock_key __key;                       \
		__raw_spin_lock_init_static_fds(spinlock_check(_lock), \
...
DEFINE_LOCK_GUARD_1(raw_spinlock, raw_spinlock_t, raw_spin_lock(_T->lock),
...
DEFINE_LOCK_GUARD_1(spinlock, spinlock_t, spin_lock(_T->lock),
		    spin_unlock(_T->lock))

DEFINE_LOCK_GUARD_1_COND(spinlock, _try, spin_trylock(_T->lock))

DEFINE_LOCK_GUARD_1(spinlock_irq, spinlock_t, spin_lock_irq(_T->lock),
		    spin_unlock_irq(_T->lock))

DEFINE_LOCK_GUARD_1_COND(spinlock_irq, _try, spin_trylock_irq(_T->lock))
...
DEFINE_LOCK_GUARD_1(read_lock, rwlock_t, read_lock(_T->lock),
		    read_unlock(_T->lock))

DEFINE_LOCK_GUARD_1(read_lock_irq, rwlock_t, read_lock_irq(_T->lock),
...
DEFINE_LOCK_GUARD_1(write_lock, rwlock_t, write_lock(_T->lock),
		    write_unlock(_T->lock))

DEFINE_LOCK_GUARD_1(write_lock_irq, rwlock_t, write_lock_irq(_T->lock),
