...
DECLARE_PER_CPU(unsigned long *[TIMING_NUM], BucketTimingstats_percpu_locktime);
...
extern void locktime_init_timing_stats(void);

extern void locktime_print_timing_stats(void);

extern void locktime_clear_timing_stats(void);
...
static inline void update_timing_stats(unsigned long diff, int name)
{
	...
	if (diff < TIME_UPPER_BOUND) {
		...
		(*this_cpu_ptr(&BucketTimingstats_percpu_locktime[name]))
			[(*this_cpu_ptr(&bucket_counter[name])) % N_BUCKETS] = diff;
	...
	do {                                                                \
		if (*this_cpu_ptr(&do_timing)) {                            \
			...
			update_timing_stats((end - (*this_cpu_ptr(&name))), \
					    name##_t);                      \
			*this_cpu_ptr(&name) = KOMB_UINT64_MAX;                  \
		}                                                           \
	} while (0)

#define LOCK_END_TIMING_PER_CPU_CS_LOOP(name, start_name)                  \
	do {                                                               \
		if (*this_cpu_ptr(&do_timing)) {                           \
			timing_t end;                                      \
			barrier();                                         \
			...
			barrier();                                         \
			update_timing_stats((end - start_name), name##_t); \
			*this_cpu_ptr(&name) = KOMB_UINT64_MAX;                 \
		}                                                          \
	} while (0)
