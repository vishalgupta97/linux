...
SYSCALL_DEFINE0(komb_start_timing) {
	...
	printk(KERN_ALERT "======== start lock timing stats ========\n");

	for_each_possible_cpu(cpu) {
		*per_cpu_ptr(&do_timing, cpu) = true;
	}

	return 0;
}

SYSCALL_DEFINE0(komb_stop_timing) {
	int cpu;

	printk(KERN_ALERT "======== stop lock timing stats ========\n");

	for_each_possible_cpu(cpu) {
		*per_cpu_ptr(&do_timing, cpu) = false;
	}
