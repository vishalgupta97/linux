...
void print_komb_stats(void)
{
	...
	for_each_online_cpu(i) {
		...
		total_counters[4] += per_cpu(fixing_preempt_count, i);
		...
		total_counters[13] += per_cpu(rwsem_percpu_reads, i);
	...
	printk(KERN_ALERT "fixing_preempt_count: %ld\n", total_counters[4]);
	...
	printk(KERN_ALERT "rwsem_percpu_reads: %ld\n", total_counters[13]);
...
SYSCALL_DEFINE0(komb_clear_stats)
{
	...
	for_each_online_cpu(i) {
		...
		*per_cpu_ptr(&fixing_preempt_count, i) = 0;
		...
		*per_cpu_ptr(&rwsem_percpu_reads, i) = 0;
