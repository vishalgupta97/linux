...
static int num_md_threads = 1;
static int md_num_cores_per_socket = 1;
...
run_combiner(struct kombd_mutex *lock, struct kombd_mutex_node *curr_node)
{
	...
	KOMB_BUG_ON((smp_processor_id() % md_num_cores_per_socket) != 0);
...
{
#if NUMA_AWARE
	return (md_num_cores_per_socket * numa_node_id()); //+
	//	((u64)lock % num_md_threads_per_socket));
#else
	return 0;
#endif
}
...
__kombd_mutex_lock_slowpath(struct kombd_mutex *lock)
{
	struct kombd_mutex_node *curr_node, *next_node;
...
#if NUMA_AWARE
	num_md_threads = num_online_nodes();
#else
	num_md_threads = 1;
#endif

	dthreads = vzalloc(num_md_threads * sizeof(struct task_struct *));
	for (i = 0; i < num_md_threads; i++) {
		dthreads[i] =
			kthread_create(komb_md_thread, NULL, "komb_thread");
		kthread_bind(dthreads[i], i * md_num_cores_per_socket);
		if (dthreads[i])
...
kombd_mutex_lock_slowpath(struct kombd_mutex *lock)
{
...
kombd_mutex_lock(struct kombd_mutex *lock)
{
	...
	curr_node = get_kombd_mutex_node(lock);
	KOMB_BUG_ON(curr_node == NULL);
	/*if ((smp_processor_id() % md_num_cores_per_socket) == 0) {
		int j = 0;
		for (j = 0; j < 7; j++)
			if (current->komb_lock_addr[j] != NULL)
				break;
...
__attribute__((noipa)) noinline notrace void
kombd_mutex_unlock(struct kombd_mutex *lock)
{
	void *incoming_rsp_ptr, *outgoing_rsp_ptr;
	struct kombd_mutex_node *curr_node;
#if WWJUMP
