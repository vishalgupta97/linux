...
__always_inline bool check_exit_condition(struct komb_node *my_node) {
...
run_combiner(struct qspinlock *lock, struct komb_node *curr_node)
{
	...
	if(check_exit_condition(curr_node)) {
		set_locked(lock);
...
__attribute__((noipa)) noinline notrace void
__komb_spin_lock(struct qspinlock *lock)
{
	u32 val, cnt;
