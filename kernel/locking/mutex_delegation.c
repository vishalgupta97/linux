...
head_of_queue:
		...
		rq_tail =
			per_cpu_ptr(&mutex_rq_tail, select_delegation_cpu(lock));
...
md_mutex_lock_slowpath(struct mutex *lock)
{
	...
	asm volatile("callq %P0\n"
		     "movq %%rsp, %c1(%%rax)\n"
		     :
		     : "i"(get_komb_mutex_node), "i"(offsetof(struct mutex_node, rsp))
		     : "memory");
	asm volatile("callq %P0\n"
		     "movq (%%rax), %%rsp\n"
		     :
...
md_mutex_lock(struct mutex *lock)
{
	if (((smp_processor_id() % num_cores_per_socket) == 0) ||
		...
		if (current->komb_curr_waiter_task) {
			struct mutex_node *curr_node = ((struct task_struct *)current->komb_curr_waiter_task)->komb_mutex_node;
...
tdlock_mutex_run_combiner(struct mutex *lock, struct mutex_node *curr_node)
{
	...
	BUG_ON(current->komb_curr_waiter_task != NULL); //TODO: Needed only for nested combining
...
static int __init md_init(void)
{
	...
	for (i = 0; i < num_delegation_threads; i++) {
		dthreads[i] = kthread_create(komb_mutex_thread, NULL, "komb_mutex_thread");
