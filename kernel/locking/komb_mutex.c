...
inline void park_waiter(struct komb_mutex_node *node)
{
...
inline void wake_up_waiter(struct komb_mutex_node *node)
{
...
__always_inline void mutex_clear_locked_set_completed(struct komb_mutex_node *node)
{
	WRITE_ONCE(node->completed, KOMB_WAITER_PROCESSED);
	WRITE_ONCE(node->locked, 0);
}

__always_inline void mutex_set_locked(struct komb_mutex *lock)
{
	WRITE_ONCE(lock->locked, 1);
}

__attribute__((noipa)) noinline notrace uint64_t
mutex_get_shadow_stack_ptr(struct komb_mutex *lock)
{
	return &current->komb_stack_curr_ptr;
}

__attribute__((noipa)) noinline notrace struct komb_mutex_node *
get_komb_mutex_node(struct komb_mutex *lock)
{
	return ((struct komb_mutex_node *)(current->komb_mutex_node));
}

__always_inline void mutex_add_to_local_queue(struct komb_mutex_node *node)
{
...
inline bool check_irq_node(struct komb_mutex_node *node)
{
	return (node->socket_id == IRQ_NUMA_NODE || node->rsp == 0xdeadbeef);
}

__always_inline struct komb_mutex_node *
mutex_get_next_node(struct komb_mutex_node *my_node)
{
...
mutex_execute_cs(struct komb_mutex *lock, struct komb_mutex_node *curr_node)
{
...
__attribute__((noipa)) noinline notrace void __mutex_lock(struct komb_mutex *lock)
{
	preempt_disable();
	...
	if (current->komb_curr_waiter_task) {
		...
		if ((struct komb_mutex *)curr_node->lock == lock) {
			...
			struct komb_mutex_node *next_node = mutex_get_next_node(curr_node);
			if (next_node == NULL)
				current->komb_next_waiter_task = NULL;
			else
				current->komb_next_waiter_task =
...
__always_inline void komb_mutex_lock(struct komb_mutex *lock)
{
	int ret;
...
EXPORT_SYMBOL(komb_mutex_trylock);

__attribute__((noipa)) noinline notrace void komb_mutex_unlock(struct komb_mutex *lock)
{
	void *incoming_rsp_ptr, *outgoing_rsp_ptr;
...
EXPORT_SYMBOL(komb_mutex_unlock);
